...
export class VacanteComponent implements OnInit {
  ...
  this.session.session$.subscribe({
    ...
    this.vacanteForm = this.fb.group({
      ...
      institucionId: [this.perfilUsuario?.institutionId, Validators.required],
...
limpiarFormulario(): void {
  this.vacanteForm.reset({
    userId: this.perfilUsuario?.id,
    institucionId: this.perfilUsuario?.id,
    provinciaId: null,
    categoriaId: null,
    titulo: '',
    tipoContrato: ITipoContrato.TiempoCompleto,  // Valor por defecto
    salarioCompensacion: '',
